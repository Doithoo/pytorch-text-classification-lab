...
Kaggle 三模型 runner 位于 `docs/recorded-run/kaggle-comparison/`。`0.3.0` 运行已经完成，详见[三模型对比参考运行](../recorded-run/kaggle-agnews-model-comparison-v0.3.0/README.zh-CN.md)。未来再次运行应保留三个运行目录、comparison、精确 revision 和失败记录，不只发布获胜模型。
