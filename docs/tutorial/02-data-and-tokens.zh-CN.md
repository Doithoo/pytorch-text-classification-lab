...
源 CSV 的每行是 `label,title,description`。准备阶段把标题和描述拼接，标签从 1-4 转为 0-3，并按类别分层抽取验证集。自有数据使用 `generic_csv` adapter 和带表头的 `text,label` 文件，标签顺序从训练集稳定生成。两种 adapter 输出的 `train.csv`、`valid.csv`、`test.csv` 字段都固定为 `id,text,label,label_id`；`dataset.json` 保存 adapter、标签顺序、样本数、随机种子、源哈希和 manifest 哈希。
...
先运行 `inspect-data` 查看长度分位数、截断数量和标签分布；它还生成 `inspection.json`，记录重复文本、跨 split 泄漏、冲突标签和 OOV。若大量文本被截断，再在显存和速度允许范围内调整 `max_length`。自有数据步骤见[使用自己的数据](../guides/using-your-data.zh-CN.md)，文件协议见[数据格式参考](../reference/dataset-format.zh-CN.md)。
